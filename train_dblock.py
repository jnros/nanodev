"""DiffusionBlocks AR training script.  Delta from train.py:
  - imports GPTDBlock; adds num_dblocks config var
  - estimate_loss returns edm + ce losses
  - training loop unpacks (logits, loss, _)
...
if init_from == 'scratch':
    ...
    raise ValueError(f"init_from={init_from} not supported for dblock")
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        if wandb_log:
            wandb.log({
                ...
                "val/loss":    losses['val'],
                "train/ce":    losses['train_ce'],
                "val/ce":      losses['val_ce'],
                "lr":          lr,
                "mfu":         running_mfu * 100,
            })
...
if master_process:
    ...
    print(f"saved dblock_summary.json + loss_curves.json to {out_dir}")
